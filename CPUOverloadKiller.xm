...
@property NSUInteger threshold, duration, idle, nearThresholdRatio, nearSample, exceedSample;
@property pid_t pid;
...
static void sample(GCGState *s,uint64_t now) {
    ...
    }else{s.exceeding=NO;s.exceeded=0;publish(s,AwemeCPUGuardStatusMonitoring,percent,0);double nearBoundary=s.threshold*(double)s.nearThresholdRatio/100.0;s.due=now+(percent>=nearBoundary?s.nearSample*NSSEC:s.idle*NSSEC);}
}
